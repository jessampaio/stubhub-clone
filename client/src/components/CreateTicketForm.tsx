...
const CreateTicketForm = () => {
  ...
  const getEvents = () => {
    axios
      .get('http://ec2-3-82-103-251.compute-1.amazonaws.com:3001/events')
  ...
  const getInfoAboutEvent = (eventId: number) => {
    axios
      .get(`http://ec2-3-82-103-251.compute-1.amazonaws.com:3001/${eventId}`)
      .then(function (response) {
  ...
  const handleCreateTicket = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    axios
      .post('http://ec2-3-82-103-251.compute-1.amazonaws.com:3001/tickets', { eventSelected, tickets })
      .then(function (response) {
        if (response.data) {
          showToast()
          setEventSelected('')
  ...
  const getInfoAboutSection = async (venueId: any) => {
    const response = await axios.get(`http://ec2-3-82-103-251.compute-1.amazonaws.com:3001/seats/sectionseats/${venueId}`)
